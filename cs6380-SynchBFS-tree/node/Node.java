...
public class Node {
	...
	class NodeLookup {
		...
		HashMap<Integer, Integer> node_index_matrix;
		public NodeLookup(HashMap<Integer, String> map, HashMap<Integer, Integer> pos) {
			this.map = map;
			this.node_index_matrix = pos;
		}
		
		public int getIndex(int id) {
			return node_index_matrix.get(id);
		}
	...
	public Node(String[] args) {
		...
		HashMap<Integer, String> map = FileUtil.readConfig("config.txt", String.valueOf(id), neighbors, pos);
		nodeLookup = new NodeLookup(map, pos);
		...
		numOfNodes = nodeLookup.map.size();
	}
	 ...
	 */
	public synchronized void broadcastBFSTreeMessage(String type, Integer except) {
		if (type.equals(MsgType.QUERY)) {
			for (int neighbor : neighbors) {
				if (except != null && except == neighbor) {
					continue;
				}
				int receviver = nodeLookup.getIndex(neighbor);
				int sender = nodeLookup.getIndex(id);
				tree.getClock().local_increase(sender, receviver);
			}
			...
			for (int neighbor : neighbors) {
				...
				String ip = nodeLookup.getIP(neighbor);
				String port = nodeLookup.getPort(neighbor);
				BFSTreeMessage message = new BFSTreeMessage(id, neighbor, type, tree.getClock().getMatrix());
				System.out.println(id + "BFSTreeMessage broadcast:" + message);
				// connect to neighbor and send a message, then close connection immediately
				try (Socket socket = new Socket(ip, Integer.parseInt(port))) {
					ObjectOutputStream oos = new ObjectOutputStream(socket.getOutputStream());
					oos.writeObject(message);
	 ...
	 */
	public synchronized void sendPrivateMessage(BFSTreeMessage message) {
	 ...
	 */
	public synchronized void processBFSTreeMessage(BFSTreeMessage message) {
		String type = message.getType();
		if (type.equals(MsgType.QUERY)) {
			tree.processBFSQuery(message);
		} else if (type.equals(MsgType.REJECT)) {
			tree.processReject(message);
		} else if (type.equals(MsgType.ACCEPT)) {
			tree.processAccept(message);
		}
		//current node finish all its work in building bfs tree
		if (tree.check(neighbors, tree.getParent())) {
			...
			//start to prepare to find max degree node
			tree.init();
		}
	}
	...
	public static void main(String[] args) {
		if(args == null || args.length <= 0) {
			System.err.println("Please Input Node Id According to the config.txt");
			System.exit(-1);
			return;
		}
